...
INSERT INTO Staff (StaffID, Name, Role, LicenseExpiryDate) VALUES (1001, 'Jacob', 'Doctor', TO_DATE('2026-02-01','YYYY-MM-DD'));
INSERT INTO Staff (StaffID, Name, Role, LicenseExpiryDate) VALUES (1002, 'Mac', 'Nurse', TO_DATE('2029-06-11','YYYY-MM-DD'));
INSERT INTO Staff (StaffID, Name, Role, LicenseExpiryDate) VALUES (1003, 'Irina', 'Nurse', TO_DATE('2023-05-10','YYYY-MM-DD'));
INSERT INTO Staff (StaffID, Name, Role, LicenseExpiryDate) VALUES (1004, 'Chris', 'Doctor', TO_DATE('2027-12-22','YYYY-MM-DD'));
INSERT INTO Staff (StaffID, Name, Role, LicenseExpiryDate) VALUES (1005, 'Lily', 'Doctor', TO_DATE('2022-8-05','YYYY-MM-DD'));
...
INSERT INTO Scheduling (SchedulingID, PatientID, StaffID, RoomID, ScheduledDate) VALUES (20005, 10002, 1004, 902,  TO_DATE('2022-08-05','YYYY-MM-DD'));
INSERT INTO Scheduling (SchedulingID, PatientID, StaffID, RoomID, ScheduledDate) VALUES (20006, 10003, 1005, 903,  TO_DATE('2023-09-11','YYYY-MM-DD'));
...
INSERT INTO Scheduling (SchedulingID, PatientID, StaffID, RoomID, ScheduledDate) VALUES (20008, 10005, 1001, 905,  TO_DATE('2021-12-31','YYYY-MM-DD'));
...
CREATE TABLE Schedules (
    ...
    PatientID INT,
    ScheduledDate DATE,
    AppointmentTime TIMESTAMP, -- Use TIMESTAMP instead of TIME
    FOREIGN KEY (StaffID) REFERENCES Staff(StaffID),
    FOREIGN KEY (RoomID) REFERENCES Rooms(RoomID),
    FOREIGN KEY (PatientID) REFERENCES Patients(PatientID)
);
...
-- DROP TABLE IncomeReport;
-- DROP TABLE Salary;
-- DROP TABLE LeaseSchedule;
-- DROP TABLE MonthlyCosts;
...
-- QUE 1
SELECT avg(Amounts) as AverageAmount, avg(ProcedureDuration) as AverageProcedureDuration
FROM Treatment T
JOIN Procedures P ON P.ProcedureID = T.ProcedureID
JOIN BillingRecord A ON T.PatientID=A.PatientID 
WHERE A.TreatmentDate BETWEEN DATE '2021-02-01' AND DATE '2021-02-20';


-- QUE 2
SELECT SUM(Amounts) AS TotalIncome
FROM Treatment T
JOIN Procedures P ON P.ProcedureID = T.ProcedureID
JOIN BillingRecord A ON T.PatientID=A.PatientID 
WHERE A.TreatmentDate=DATE '2021-02-01'
...
FROM 
    Staff S
WHERE S.StaffID IN (SELECT C.StaffID FROM Capabilities C WHERE 
C.Skill IN ('File taxes', 'Meet the press', 'Organize spring cleaning', 'Do teeth cleaning', 'Reorder inventory')
GROUP BY 
C.StaffID
HAVING 
COUNT(DISTINCT C.Skill) = 5
);


-- QUE 4--Total Number of Patients Treated by Each Doctor
SELECT S.Name AS DoctorName,
       COUNT(DISTINCT SR.PatientID) AS TotalPatientsTreated
FROM Staff S
JOIN Scheduling SR ON S.StaffID = SR.StaffID
GROUP BY S.Name;
-- Upcoming Appointments for a Specific Doctor
SELECT S.Name AS DoctorName,
       COUNT(SR.SchedulingID) AS UpcomingAppointments
FROM Staff S
JOIN Scheduling SR ON S.StaffID = SR.StaffID
